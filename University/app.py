...
if st.button("Display Certificate"):
    ...
    st.write(f"The certificate ID is {certificate_id}")
    st.write(f"The certificate's tokenURI metadata is {certificate_uri}")
    st.image("https://storage.googleapis.com/production-ipage-v1-0-6/366/1396366/3SuIdNP0/a1517b1200b64a168713838880f00e89")



    file = open('read.txt', 'w')
    #file.write(f"Certificate URI: {certificate_uri}")
    file.write(f"Certificate URI: {certificate_uri}")
    file.close()

    os.system("lpr -P Canon_MX490_series read.txt")
